...
class EncoderRegressionHeadV2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3):
        super().__init__()
        ...
        self.linear = nn.Linear(in_channels, 2, bias=True)

    def forward(self, X, X_city, X_gsd):
        X = self.pool(X)
        X = self.flatten(X)
        # => (4,2048)
        X_gsd = X_gsd.unsqueeze(1)
        X = torch.cat([X, X_city, X_gsd], axis=1)
