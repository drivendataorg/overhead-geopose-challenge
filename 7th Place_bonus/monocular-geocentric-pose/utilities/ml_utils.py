...
# mod: import 2 new decoders
from utilities.deeplabv3_vflow import DeepLabV3VFLOW
from utilities.fpn_vflow import FPNVFLOW
...
class Epoch:
    def __init__(
        ...
        self.accumulation_steps = args.accum_steps # mod: assign accumulation steps
    ...
    def run(self, dataloader):
        ...
        with tqdm(
            ...
            for batch_id, itr_data in enumerate(iterator): # mod: introducing batch_id needed for gradient accumulation
                ...
                if self.stage_name != "valid":
                    ...
                    do_step = ((batch_id+1) % self.accumulation_steps == 0) # mod: find out if we will do optimizer step
                    ...
                    (
                        ...
                        mag_pred,
                        scale_pred,
                    ) = self.batch_update(image, y, do_step=do_step) # mod: pass do_step further
...
class TrainEpoch(Epoch):
    def __init__(
        ...
        self.scaler = torch.cuda.amp.GradScaler(enabled=bool(self.args.amp_enabled)) # mod: amp

    def on_epoch_start(self):
        self.model.train()

    def batch_update(self, x, y, do_step=True):
        self.optimizer.zero_grad()

        with torch.cuda.amp.autocast(enabled=bool(self.args.amp_enabled)): # mod: amp
            ...
            loss_combined = loss_combined / self.accumulation_steps # mod: for gradient accumulation


        # loss_combined.backward() # old
        self.scaler.scale(loss_combined).backward() # mod: amp

        if do_step: # mod: waiting for several backward passes before making optimizer.step()
            # self.optimizer.step() # old
            self.scaler.step(self.optimizer)
            self.scaler.update()
            # print('Calling optimizer.step()...') # mod: check if we accumulating gradients
...
def train(args):
    ...
    # mod: load model weights to continue training from
    if args.model_path:
        model.load_state_dict(torch.load(args.model_path))
        print('Continue training from:', args.model_path)
...
def build_model(args):
    if args.decoder == 'unet':
        ...
        model = DeepLabV3VFLOW(args.backbone, encoder_weights=args.encoder_weights)
    elif args.decoder == 'fpn':
        model = FPNVFLOW(args.backbone, encoder_weights=args.encoder_weights)
    return model
