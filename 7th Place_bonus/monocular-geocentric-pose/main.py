...
if __name__ == "__main__":
    ...
    # mod: change bool type to int because when using bool any string evaluates to True
    parser.add_argument(
    ...
    # mod: add several addtional command line arguments below
    parser.add_argument(
        "--amp_enabled",
        type=int,
        choices=[0, 1],
        help="Enable/disable automatic mixed precision for training and validation. Inference is not affected (always in full precision)",
        default=0,
    )
    parser.add_argument(
        "--accum_steps",
        type=int,
        help="Number of steps to accumulate gradients. 1 means NO accumulation",
        default=1,
    )
    parser.add_argument(
        "--encoder_weights",
        type=str,
        help="Name of the weights used to initialized encoder and preprocessing function",
        default="imagenet",
    )
    parser.add_argument(
        "--decoder",
