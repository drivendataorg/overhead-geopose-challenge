...
class Pipeline:
    ...
    def build_criterions(self, config: Dict) -> Tuple[Dict[str, nn.Module], List[Callback]]:
        ...
        callbacks.append(MetricAggregationCallback(prefix="loss", metrics=losses, mode=losses_aggregation))
    ...
    def build_metrics(self, config) -> List[Callback]:
        return []
    ...
    def get_scheduler(
        ...
        **kwargs
        ...
        if name == "cos":
            ...
            scheduler = FlatCosineAnnealingLR(
                optimizer, T_max=num_epochs, T_flat=int(num_epochs * flat_fraction), eta_min=min_learning_rate
            )
        elif name == "plateau":
            ...
            scheduler = OneCycleLRWithWarmup(
                optimizer, lr_range=(learning_rate, min_learning_rate), num_steps=batches_in_epoch * num_epochs, **kwargs
            )
        elif name == "exp":
            ...
            self.master_print(f"Using {name} scheduler with milestones", milestones)
        elif name == "simple":
    ...
    def get_optimizer(
        ...
        if optimizer_name == "ranger21":
            optimizer_cls = partial(Ranger21, num_epochs=num_epochs, num_batches_per_epoch=batches_in_epoch)
    ...
    def build_model(self, config: Dict) -> nn.Module:
        ...
        elif self.cfg.transfer_encoder_decoder:
            transfer_checkpoint = fs.auto_file(self.cfg.transfer_encoder_decoder)
            self.master_print("Transferring encoder/decoder from model checkpoint", transfer_checkpoint)
            checkpoint = load_checkpoint(transfer_checkpoint)
            pretrained_dict = checkpoint["model_state_dict"]

            enc_dec = collections.OrderedDict(
                (k, v) for (k, v) in pretrained_dict.items() if (k.startswith("encoder.") or k.startswith("decoder."))
            )

            transfer_weights(model, enc_dec)
    ...
    def get_experiment_dir(self, config) -> str:
        ...
        experiment_slug = f"{current_time}_{config.model.config.slug}"

        fold = config["dataset"].get("fold", None)
        if fold is not None:
            experiment_slug += f"_fold{fold}"
        down = config.train.downsample
        if down > 1:
            experiment_slug += f"_down{down}"
    ...
    def train(self):
        ...
        metric_callbacks = self.build_metrics(config)
    ...
    def build_quircks(self, config: DictConfig) -> List[Callback]:
        callbacks = hydra.utils.call(config["quircks"])
        if callbacks is None:
            return []
        if len(callbacks):
            self.master_print("Additional callbacks")
        for c in callbacks:
            self.master_print("  ", c)
        return callbacks
